...
func MakeHTTPHandler(endpoints Endpoints, options ...httptransport.ServerOption) http.Handler {
	...
	m.Methods("GET").Path("/auth/jwks").Handler(httptransport.NewServer(
		endpoints.JWKSEndpoint,
		DecodeHTTPJWKSZeroRequest,
	...
	m.Methods("HEAD").Path("/auth/login").Handler(httptransport.NewServer(
		endpoints.LoginEndpoint,
	...
	m.Methods("HEAD").Path("/auth/refresh").Handler(httptransport.NewServer(
		endpoints.RefreshEndpoint,
		DecodeHTTPRefreshOneRequest,
		EncodeHTTPGenericResponse,
		serverOptions...,
	))
...
// DecodeHTTPJWKSZeroRequest is a transport/http.DecodeRequestFunc that
// decodes a JSON-encoded jwks request from the HTTP request
// body. Primarily useful in a server.
func DecodeHTTPJWKSZeroRequest(_ context.Context, r *http.Request) (interface{}, error) {
	defer r.Body.Close()
	var req pb.JWKSRequest
...
func DecodeHTTPLoginOneRequest(_ context.Context, r *http.Request) (interface{}, error) {
	...
	if EmailLoginStrArr, ok := queryParams["email"]; ok {
		EmailLoginStr := EmailLoginStrArr[0]
		EmailLogin := EmailLoginStr
		req.Email = EmailLogin
	}
...
// DecodeHTTPRefreshOneRequest is a transport/http.DecodeRequestFunc that
// decodes a JSON-encoded refresh request from the HTTP request
// body. Primarily useful in a server.
func DecodeHTTPRefreshOneRequest(_ context.Context, r *http.Request) (interface{}, error) {
	defer r.Body.Close()
	var req pb.RefreshRequest
	buf, err := ioutil.ReadAll(r.Body)
	if err != nil {
	...
	if RefreshTokenRefreshStrArr, ok := queryParams["refresh_token"]; ok {
